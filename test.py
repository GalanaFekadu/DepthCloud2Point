...
if __name__ == '__main__':
    ...
    DepthCloud2Point = DepthCloud2Point(initial_point=settings.initial_point[0]).to(device)
    checkpoint = torch.load(settings.model_path)
    DepthCloud2Point.load_state_dict(checkpoint['model_state_dict'])
    ...
    show_3d(DepthCloud2Point.initial_point, path=plotly_path.joinpath('initial_point.html'))

    loss_test = 0
   DepthCloud2Point.train(mode=False)
